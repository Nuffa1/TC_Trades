...
class Backtester:
    ...
    def run(self, initial_capital: float = 1_000_000) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Trade log: capture rebalance events
        trade_log: List[Dict] = []
        ...
        for i, date in enumerate(dates[1:], start=1):
            ...
            if date in rebal_dates_set:
                ...
                if not sector_prices.empty:
                    ...
                    selected_tickers = allowed
                    new_weights = self.pc.build_portfolio(allowed)
                    weights_dict = new_weights
                else:
                    new_weights = {}
                ...
                # Log this rebalance event
                trade_log.append({
                    'date': date,
                    'event_type': 'rebalance',
                    'regime': regime.name if regime else 'UNKNOWN',
                    'sectors_available': ', '.join(available_sectors),
                    'tickers_selected': ', '.join(selected_tickers),
                    'weights': weights_dict,
                    'turnover': turnover,
                    'portfolio_value': equity.iloc[i - 1]
                })
        ...
        return equity.to_frame(name="equity"), trade_log_df
...
def run_pipeline(
    ...
    equity_curve, trade_log = backtester.run(initial_capital=1_000_000)

    return equity_curve, regime_series, trade_log
